...
{
    if (strcmp(hash_string, p1))
    {
        if (strcmp(hash_string, p2))
        {
            if (strcmp(hash_string, p3))
            {
                if (strcmp(hash_string, p4))
                {
                    return 0;
                }
                return 4;
    ...
    for (a = start; a <= end; a++)
    {
        for (b = start; b <= end; b++)
        {
            for (c = start; c <= end; c++)
            {
                for (d = start; d <= end; d++)
                {
                    sprintf(temp, "%c%c%c%c", a, b, c, d);
                    digest = SHA256(temp, strlen(temp), 0);
...
void omp_find_pass()
{
    ...
    char *hash_string;
    char *temp = malloc(sizeof(char) * 4);
#pragma omp parallel for collapse(4) //shared(t) //private(temp)
    for (a = start; a <= end; a++)
    {
        for (b = start; b <= end; b++)
        {
